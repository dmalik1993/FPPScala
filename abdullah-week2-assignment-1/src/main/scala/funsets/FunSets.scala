...
trait FunSets extends FunSetsInterface:
   ...
   */
  def singletonSet(elem: Int): FunSet = newElem => elem == newElem
  ...
  def forall(s: FunSet, p: Int => Boolean): Boolean =
    def iter(a: Int): Boolean =
      if contains(s, a) && a >= -bound && !p(a) then
        false
...
//      if contains(s, a) && a >= -bound && p(a) then
//        true
//      else if (!contains(s, a) || !p(a)) && a >= -bound then
//        iter(a - 1)
//      else
//        false 
//    iter(bound)
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = elem1 => exists(s, elem2 => f(elem2) == elem1)
