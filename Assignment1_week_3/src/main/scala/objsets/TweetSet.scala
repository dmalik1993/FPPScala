...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.exists(g => tweet.text.contains(g)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => apple.exists(a => tweet.text.contains(a)))
